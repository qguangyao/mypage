...
const checkList = ['星期一', '星期二', '星期三', '星期四', '星期五', '星期六', '星期日'];
...
export default () => {
  ...
  const cal = () => {
    ...
    else if (positionTime >= pmEndTime)
      pastDay++;
  ...
  return (
    <div className={Styles.rootbackground}>
      <div><Text>您每周的工作时间：</Text> <Divider type={"vertical"} />
      ...
      <div>
        <Divider />
        <CheckboxGroup options={checkList} value={checkedList} onChange={onChange} />
        <Divider /></div>
      <div>
        {/* <h1 className={styles.title}>Page worktime/index</h1> */}
        <Text>您上午的上班时间</Text><Divider type={"vertical"} /><Text>开始：</Text>
        <TimePicker onChange={(time, timeString) => onchange(time, timeString, 0)} defaultValue={tb.amStart} format={format} />
        <Divider type={"vertical"} />
        <Text>结束：</Text>
        <TimePicker onChange={(time, timeString) => onchange(time, timeString, 1)} defaultValue={tb.amEnd} format={format} />
        <Divider />
      </div>
      <div>
        <Text>您下午的上班时间</Text><Divider type={"vertical"} /><Text>开始：</Text>
        <TimePicker onChange={(time, timeString) => onchange(time, timeString, 2)} defaultValue={tb.pmStart} format={format} />
        <Divider type={"vertical"} />
        <Text>结束：</Text>
        <TimePicker onChange={(time, timeString) => onchange(time, timeString, 3)} defaultValue={tb.pmEnd} format={format} />
        <Divider />
      </div>
      <div>
        <Button type="primary" onClick={(tb) => cal(tb)}>计算</Button><Divider type={"vertical"} /><Text>您本周工作时间已过去：{persentText}%</Text>
      </div>
    </div>
  );
}
